...
BUILD_CALYPSO = CALYPSO_ENV=$(CALYPSO_ENV) MINIFY_JS=$(MINIFY_JS) NODE_ARGS=$(NODE_ARGS) npm run -s build
BUILD_DESKTOP = NODE_PATH=calypso/server:calypso/client CALYPSO_SERVER=true npx webpack --config webpack.config.js

ifeq ($(OS),Windows_NT)
	BUILD_CALYPSO_CMD = wsl-bash -i "./wsl-build-calypso.sh"
	BUILD_DESKTOP_CMD = wsl-bash -i -c "$(BUILD_DESKTOP)"
else
	BUILD_CALYPSO_CMD = pushd calypso && $(BUILD_CALYPSO) && popd
	BUILD_DESKTOP_CMD = $(BUILD_DESKTOP)
endif
